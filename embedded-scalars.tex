...
We frame our assessment in terms of a competition between three
different related models: semantics only, arbitrary lexical
refinements, and highly constrained neo-Gricean refinements.  Our goal
is to see which instantiation achieves the best match with the human
response pattern described above. Our rationale for this approach is
...
patterns are broadly in agreement with intuitions that guided the
...
achieve significant correlations at $p < 0.001$, whether assessed with
Pearson's correlation coefficient or Spearman's rank correlation.
Intuitively, we know that the null hypotheses given to use by
frequentist statistical tests are false, so we pursue a strategy that
is more likely is more stringent and more likely to provide insights
into the quality of the underlying model. 
...
experiment. Similarly, the set of messages $\Messages$ corresponds
directly to \eg{expmsgs}, with some adjustments to keep the logical
grammar simple. We stipulate flat priors and even costs. As noted in
...
assumptions about these values, this risks of overfitting to the
particular data we have and thus overstating the true accuracy of the
model.

The three models we consider are defined as follows:
%
\begin{examples}
\item 
  \begin{examples}
  ...
  % \item Rational Speech Acts (RSA): the predicted values are the
...
We adopt three methods of comparison: Pearson's correlation
...
\begin{figure}[t]
  \centering
  \includegraphics[width=1\textwidth]{fig/allmodels2.pdf}
...
\begin{figure}[htp]
  ...
  \begin{tabular}[c]{r c c c}
    ...
    %Rational Speech Acts      & 0.92 & 0.75 & 0.0094\\
...
the human responses, and \figref{fig:results} gives the overall
...
more interest, though. Two paterns stand out.

First, unconstrained uncertainty seems to be the least accurate of the
three models. Although it achieves lower MSE than the literal
semantics, its overall correlations are lower. We saw in connection
with \figref{fig:subjects} that the unintuitive lexica that this model
entertains can lead to implausible final patterns. We conjeture that
this is the root cause of the model's poor performance here.

Second, neo-Gricean uncertainty achieves the best results across
...
The overall analysis give in \figref{fig:overall} understates the
...
and middle columns essentially allow for only literal interpretations,
leaving just the right column for our pragmatic models to shine. It's
thus worth homing in on just those cases and assessing each in
isolation. The results of that assessment are in
\figref{fig:crucial-items}. The unconstrained uncertainty model is
somewhat redeemed here, but the neo-Gricean model emerges as the best
model for explaining the human data. COMPLETE THIS AND ADD THE TABLES.

\begin{figure}[htp]
  \centering
  \begin{tabular}[c]{r *{3}{r}@{\hspace{14pt}}*{3}{r}@{\hspace{14pt}}*{3}{r} }
    \toprule
    & 
    ...
    \bottomrule
  \end{tabular}
  \caption{Results on crucial items}
