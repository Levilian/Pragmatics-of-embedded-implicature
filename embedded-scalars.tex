...
also that its predictions are highly correlated with people's actual
...
manner embraces a role for language. Neo-Griceans expand this role
...
utterance $U$ with content $p$ that falls short of this assumption,
creating a conflict. If the listener posits that the speaker actually
intended a different proposition $q$, then $U$ is reconciled with the
assumption of cooperativity.
...
maxim of quantity) is in tension with another pragmatic pressure. The
...
\word{Player~A hit all of his shots} would have been more informative?
Given our assumptions, it must be that she was prevented from using
...
possible worlds with labels like \world{NA}, which means that Player~A
...
$\entails$ as a conditional, but the definition should be thought of
...
three speakers, achieving a scalar implicature: hearing \word{Player~A
  scored} leads this listener to assume that the state is \world{S}.
...
$1$. To align these values, we rescale the human responses: if $x^{s}$
