...
% \title{Embedded implicatures, compositional uncertainty, and
% pragmatic reasoning}
...
described these inferences as \tech{conversational implicatures}: 
...
`some and not all`, as in example \eg{some1}:
...
\tech{scalar implicatures} (SIs) are generated via the same kind of
...
\tech{rational speech act} models
...
directly on the work of \posscitet{Bergen:Goodman:Levy:2012} and
...
% \citeauthor{ChierchiaFoxSpector08} do not 
...
real, however. In \secref{sec:exp1}, we describe a new experiment that
...
% true. Whether or not this assumption is valid, the need to invoke it
% opens up further room for criticism. 
...
%  As we said above, the model is a minor
...
\tabref{tab:subjects}. For the most part, they seem well-aligned with
...
in the contexts of displays like \figref{fig:exp1}.  This cover story
...
\marginnote{This section has been totally reconceived and rewritten.}
...
\begin{figure}[!h]
  ...
  \includegraphics[height=0.9\textheight]{code/temp}
...
In \tabref{tab:overall}, we summarize our overall quantitative
...
\tabref{tab:crucial-items}.  The most dramatic pattern is that the
...
distributions of \figref{fig:exp-analysis}, and it is reassuring to
