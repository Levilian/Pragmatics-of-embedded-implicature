...
\begin{abstract} 
  ...
  quantifiers can be locally enriched to impose an upper bound (`only
  some'). Such construals are predicted not to occur on Gricean
  accounts of conversational implicature, but they are easily within
  reach of recent grammar-driven theories.  We propose a model that
  both embraces the semantic insights of grammar-driven theories and
  offers pragmatic explanations for where and how implicatures
  arise. The model straightforwardly synthesizes
  \posscitet{Bergen:Levy:Goodman:2014} \tech{lexical uncertainty}
  model with a full compositional semantics. After describing the
  model, we present new experimental evidence that people arrive at
  locally-enriched construals, not just in non-monotonic environments
  but also (and more controversially) in downward-entailing ones. We
  then show not only that our model generates local enrichments, but
  also that its predictions are highly correlated with our
  experimental data, and that these predictions can be improved by
  incorporating neo-Gricean hypotheses about the nature of lexical
  alternatives.
\end{abstract}
...
for a weak statement like \word{Player~A hit some of his shots} when a
stronger statement like \word{Player~A hit all of his shots} would
...
worlds with labels like \world{NA}, which means that Player~A hit none
of his shots and Player~B hit all of his shots, and \world{SS}, which
means that both players hit some but not all of their shots. There are
$3^{2} = 9$ such worlds. The literal semantics of \eg{some} in this
context is the proposition given in line (b) of \eg{some-sem}. Our
...
\eg{everysome-sem}, which takes us to a meaning (d) that is consistent
...
excludes \world{NA} and \world{AN}, but they are essentially incapable
of `introducing' \world{SA} and \world{AS}.
...
\item\label{nosome-sem}
  ...
  \begin{tabular}[t]{@{} r@{. \ } l *{9}{c}@{\hspace{18pt}} l}
    a& Worlds:       & \world{NN} & \world{NS} & \world{NA} & \world{SN} & \world{SS} & \world{SA} & \world{AN} & \world{AS} & \world{AA} & \\
    b& Literal:      & \world{NN} &            &            &            &            &            &            &            &            & `no hit some' \\
    c& Local:        & \world{NN} &            & \world{NA} &            &            &            & \world{AN} &            & \world{AA} & `no hit only some' \\
  \end{tabular}
\end{examples}
...
\set{\sem{\word{Player A}}}$. Since \word{aced} already denotes a
singleton set, it has no space for further refinement. However,
\word{scored} has two further refinements. This gives rise to the
three lexica in the bottom row of \figref{fig:simplescalar}. The
...
original lexicon by saying \word{Player A aced} in \world{A} even
though \word{Player A scored} is also true there. The rightmost
...
The resulting listener inferences are summarized in
\figref{fig:subjects}. For the most part, they seem well-aligned with
the general view in the literature about how scalar terms interact in
contexts like this. For instance, we predict that a proper name
\word{P} will take on the exhaustified sense \word{only P}, as we
would expect given the salience of \word{every}. In turn, \word{some}
is interpreted as non-specific in virtue of the salience of the two
names, and it also leads to a scalar implicature due to the salience
of \word{every}. Perhaps the most striking finding is that the scalar
...
whether we can improve the picture in \figref{fig:subjects} by further
...
in \figref{fig:subjects-ALTstyle}. The picture is mostly unchanged,
...
presented with displays like the one in \figref{fig:exp1}. Each of the
...
\marginpar{Dan might check this paragraph to make sure the details align with the set-up he created.}
...
\marginpar{This is the paragraph justifying our experiment as more than a reproduction. We could tr to be more hard-hitting, but my heart isn't in it --- C\&S did a great job too.}
...
The only exception to this is for the sentence \word{No player hit
...
as equivalent to \word{any of his shots}. Negative concord of this
...
ratings in the \world{SSS} world. As we reviewed in
...
salient alternative \word{every player hit all of his shots} is a more
...
\figref{fig:exp1-results}. The highest ratings are in the \world{NNS}
condition, where it is true under its literal and local enrichment
construals. However, it also received high ratings in the \world{NSA}
and \world{SAA} worlds, where it is true only with local enrichment
...
local enrichment: though \world{NNN} received the highest average
ratings, indicating a preference for a literal construal, the ratings
for \world{NNA}, \world{NAA}, and \world{AAA} are fairly high (though
...
enrichment that \seccitet{Chemla:Spector:2011}{4.4.4} report for this
...
pronoun \word{his} in the embedded phrase should have forced a surface
...
assessing the ability of the model in \secref{sec:model} to predict
...
in \figref{fig:grammar}, as well as the following features of the
context:
%
\begin{examples}
\item\label{expmod}
...
\secref{sec:model}, we do not have empirical estimates of these
...
\item 
  \begin{examples}
  \item \tech{Literal semantics}: the predicted values are the output of
    $\listenerZero$, as in \subeg{agents}{l0}, as run on the messages
  ...
  \item \tech{Fixed-lexicon pragmatics}: the predicted values are the
    ...
    terms of the base lexicon in \figref{fig:grammar}.
...
\secref{sec:model}; we saw in connection with \figref{fig:subjects}
and \figref{fig:subjects-ALTstyle} that there might be substantial
empirical value in a highly constrained space of refinements.

\marginpar{Mike: perhaps rewrite with deeper thoughts on why we chose these three measures.}
...
false on the literal construal: \world{NSA} and \word{SAA}. The
...
enrichments leading to acceptability for \word{NNA}, \world{NAA}, and
\word{AAA}. Here, `Unconstrained uncertainty' is les aggressive in
calculating local enrichments and so seems better matched with the
human responses.

In \figref{fig:overall}, we summarize our overall quantitative
...
judgments. The comparative values are arguably of more interest,
though. A few patterns stand out.  First, `Fixed-lexicon pragmatics'
performs the least well overall.  Since it has been shown to add
substantial value in other areas of language and cognition, we
conclude that its particular approach to enrichment is at odds with
the patterns for embedded implicatures.  The precise causes are hard
to pin-point, but the fact that our target implicatures are not always
enrichments of the literal content is surely part of the problem.
...
dependent data. We saw in connection with \figref{fig:subjects} that
...
of the model's poor performance here as well.  Third, neo-Gricean
uncertainty achieves the best results across all three of our
models. Here again, this is consistent with our expectations based on
the large illustrative example from \secref{sec:illustrations}, where
we saw that this constrained, lexically-driven approach to choosing
refinements resulted in the best quantitative and qualitative pattern.

The overall analysis given in \figref{fig:overall} understates the
...
\figref{fig:crucial-items}.  The most dramatic pattern is that the
...
the patterns extremely well. These tight fits are already evident in
the raw distributions of \figref{fig:exp-analysis}, and it is
reassuring to see them reflected in our assessment measures as well.
...
picture. In addition, we saw that the space of refinements
(alternatives) has a significant impact on the final predictions, It
would thus be worthwhile to further explore the theoretically
moticated options in this area, seeking better fits with our own
experimental patterns and validating those conclusions in follow-up
experiments. We have made publicly available all the data and code
associated with this paper in an effort to encourage these new strands
of theory development and quantitative assessment.
