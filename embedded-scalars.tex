...
paper --- embedded scalar terms --- and the associated challenges for
Gricean accounts.
...
% For example, participants in their experiment clearly struggled with
% the complex visual displays, often giving low ratings to true
% readings and sometimes giving high ratings to false ones.
% We believe that the theoretical challenges posed by embedded
% implicatures are real, however. In \secref{sec:exp1}, we describe a
% new experiment that 

% uses simpler displays, but that reproduces the
% core findings of \citeauthor{Chemla:Spector:2011}'s studies.
...
% \marginnote{DL: changed ``always underdetermines'' to
% ``frequently'', since whether O-insertion generates ambiguity
% depends on details of the sister's denotation and its alternative
% set.}
...
the average ratings are not statistically different from 1 (the lowest
...
Turning to the critical conditions, responses for \target{every}{some}
(upper right) were strongly indicative of local enrichment. This
sentence received its highest ratings in the \world{SSS} world. As we
reviewed in \secref{sec:implicature}, example \eg{everysome-sem}, in
order to count as a complete report in this world, this sentence
...
next highest average ratings. Only a simple Gricean calculation is
required for the sentence to completely describe these conditions. The
...
basis of contextual and world knowledge.
...
both the quantitative and qualitative patterns in our experimental
data.
...
that arbitrary statistical comparisons with a null hypothesis are not
informative. As we have seen, our human response patterns are broadly
in agreement with the intuitions that guided the development of these
models, so such null hypotheses can be rejected with high confidence.
...
\begin{table}[t]
  ...
  \begin{tabular}[c]{r rrr @{\hspace{36pt}} rrr  @{\hspace{36pt}} rrr }
    \toprule
    & 
    \multicolumn{3}{c}{\target{every}{some}}{\hspace{36pt}} & 
    \multicolumn{3}{c}{\target{exactly one}{some}}{\hspace{36pt}} &
