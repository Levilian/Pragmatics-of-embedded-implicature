...
meaning and intention \citep{Grice68,Grice89,sep-grice}. In view of our experimental
...
\marginnote{Clarified the mix of pragmatic and grammatical restrictions that might govern $O$'s distribution for CFS.}
...
\marginnote{Purged the rest of this section of unnecessary references to Grice, since these offended R1.}
...
\marginnote{Added lots of more references on lexical uncertainty.}
...
and adaptable (\citealt{clark-clark79,Clark97,Lascarides:Copestake:1998,Glucksberg:2001}; for an overview and general discussion, see \citealt{Wilson:Carston:2007}). 
Thus, in our model, discourse 
...
\item\label{modobjects}
  ...
  \item\label{lexset}% 
    $\LexSet = \Set{\Lex : \Lex(\nullmsg, w)=1 \text{ for all } \state \in \Worlds 
...
\marginnote{Expanded this to clarify the role of alternative messages.}
...
\marginnote{Added new notes about the power of alternative messages.}
...
\marginnote{Revised opening sentence to explicitly say that fixed-lexicon won't suffice here.}
...
a set of visual scenes.  The participant was instructed to choose 
the scene or scenes, if any, that he or she considered ``best described by the sentence''.
...
\item\label{coverstory} We are trying to train an automated
  ...
  On each page, we will show you the outcome from a basketball
  free-throw shooting competition between 3 players, labeled A B and C
  for convenience. We'll give you a comment the system has generated,
  and your job is to decide whether a comment is true or false.
\end{examples}
...
\begin{table}[!t]
  ...
  \begin{tabular}[c]{r c@{ \hspace{8pt} }c c@{ \hspace{8pt} }c c@{ \hspace{8pt} }c}
    ...
    \bottomrule   
  \end{tabular}
  \caption{Overall assessment with 95\% confidence intervals obtained via by-subjects bootstrapping.}
...
\marginnote{Revised paragraph in response to R1.}
...
accessible. It should be noted also that the Spearman coefficient best
reflects the ability of a model to capture the preference ordering in
the human data, so the `Neo-Gricean uncertainty' model might still be
the favored choice for this case.
...
items, or applying the resulting models in new domains. Such
investigations could engage not only the nature of lexical refinement,
but also whether phrasal refinements are essential and need to be
finely controlled, for example, to properly describe the complex
distribution of what \citet{Sauerland:2012,Sauerland:2014} calls
\tech{intermediate implicatures}.  We have made publicly available all
the data and code associated with this paper in an effort to encourage
these new strands of theory development and quantitative assessment.
...
outcomes, but the findings for the other two parameters may be relevant to broader discussions of \emph{bounded} rationality in
pragmatics. First, further iteration beyond $\UncertaintyListener[1]$
is not necessary \citep{Vogel-etal:2014}. Second, the assumption in the main text that $\lambda=1$, made primarily for clarity in deriving model predictions, does not provide the optimal fit to the experimental data: the value $\lambda = 0.1$ is slightly better. 
At lower values of $\lambda$, our listeners assume that speakers are paying little attention to the informativity of their messages, seeking only to be truthful (e.g., \citealt{McMahan:Stone:2015}).
This is consistent with previous accounts according to which speakers are often unable to achieve ideal
pragmatic calculations due to the
the cognitive demands of production \citep{pechmann1989,levelt1993,engelhardt2006,dale1995,van-deemter2012,gatt2013}. At the same time, the improvement is slight --- compare \tabref{tab:grid} to \tabref{tab:binary:overall} in the main text --- and previous work has generally found that higher values of $\lambda$ provide better predictions (for example, \citealt{Kao2014,Kao-etal:2014,lassitergoodman15}).  
...
replaced by ``judgments about the quality of the comments'' and
``decide whether a comment is true or false'' replaced by ``rate how
good the comment is''. They completed the same three training items as
were used in our main experiment. The design was again
between-subjects.  Each sentence received a total of 300
