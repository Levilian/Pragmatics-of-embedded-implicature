...
\begin{abstract} 
  ...
  such terms (`only some') is predicted to be marked or impossible if
  it does not strengthen the literal meaning, whereas recent
  grammatical accounts of implicature predict such construals to be
...
of non-monotone quantifiers, as in \word{Exactly one player hit some
  of his shots}. In such cases, the interpretation enriched by an
...
this reasoning precise in the terms we just used for \eg{some}
requires more assumptions than for \eg{some}.  If we take the
implicature to be the negation of the stronger alternative \word{every
  player hit all of his shots}, then the reasoning proceeds as in the
first four lines of \eg{everysome-sem}, which takes us to a meaning
...
\eg{some}. One example of such a premise is that of uniform outcomes
...
the phrase \word{synagogues and other churches}, then her listeners
...
\word{church or synagogue}, her listeners receive a weak signal that
...
\marginnote{Corrected minor errors in this pargraph noted by R1.}
...
\marginnote{This paragraph introduces different parameter settings and introduces our appendiex on parameter exploration.}
...
\word{Player~A scored} leads this listener to assume that the most
probable state is \world{S}, and if this listener is forced to make a
categorical decision about the intended meaning of the utterance, it
would chose this enriched construal.
...
\marginnote{This para and the next are new, as is \tabref{fig:sauerland}. They seel to answer R3's concerns about ``global'' implicature.}
...
in that it fully crosses the alternatives for \word{some} with the
alternatives for \word{or}.
...
\tabref{tab:sauerland}.)\footnote{There is one unnatural aspect of the
  simulation in \tabref{tab:sauerland}: it does not allow the speaker
  ...
  \word{Player~A hit some shot or cheered} and \word{PlayerA hit some
    shot}. For a version of the model that allows for the requisite
  sort of uncertainty by closing the state space under joins, we refer
  to \citealt{Potts:Levy:2015}.}
...
\marginnote{This material was extracted from the previous section and greatly expanded. Essentially all the prose is new.}

The illustrative examples in the previous section beging to show that
...
places participants in a role comparable to that of one of our
listener agents. However, the method was criticized by
...
component of many implicatures,\footnote{\posscitet{Levinson00}
  ...
  to one of its salient or proptypical subkinds. In the context of a
...
judgments on a scale between `Yes' and `No'. The results experiments
were consistent with very high rates of local enrichment in upward and
...
we should seek our simple, naturalistic displays. Previous experiments
...
idiosynacracies of particular displays, to avoid artifacts of salience
or contrast that could stimulate responses that are consistent with
implicature calculation without requiring it.
...
presented with three training items, designed to ensure that
...
judged the same sentence twice.
...
responses (mean 80); this variation resulted from randomization of the
...
state and assess the extent to which the speaker succeeded in
conveying that state with her utterance. We also conducted a variant
of the experiment in which participants gave responses on a
seven-point Likert scale ranging from `Bad description' to `Good
description', to see whether this would reveal information about the
quality of the report. These two versions of the experiment led to
qualitatively identical outcomes. See \appendixref{app:likert} for
details.
...
$p = 0.09$ and $p = 0.04$, respectively), and of all the literally
true worlds for this condition world \world{AAA} received the lowest
percentage of `True' responses (lower than \world{SSA} and
\world{SAA}; both at $p < 0.01$).  Only a simple Gricean calculation
is required to account for the higher rate of `True' for \world{SSA}
and \world{SAA} compared with \world{AAA}: in the latter world, the
...
percentage of `True' responses is for the \world{NNS} condition, where
it is true under its literal and local enrichment construals. However,
it was also frequently judged true in the \world{NSA} and \world{SAA}
worlds (both higher at $p < 0.001$ than in \world{SSA}, the world
yielding the highest rating among those in which the sentence is false
both literally and under all possible enrichments). For \world{NSA}
and \world{SAA}, the sentence is true only with local enrichment
...
as it was generally perceived to be true in the \world{NNA} condition.
...
worlds \word{NNS} and \world{NSA}; all pairwise significance tests for
the cross-product of $\set{\world{NNS}, \world{NSA}}$ and
...
\item\label{expmod}
  ...
  \item $\Worlds = $ the set in \eg{conds}
    ...
    \marginnote{Added the missing null message to $\Messages$ -- thanks R1!}
...
but the three measures together yield a succinct multidimensional
summary of how the models fare.
...
distribution over states given messsages. The one noteworthy thing
...
\item\label{agents-extended}
  ...
  \item $\UncertaintyListener[k](\state \given \msg) \propto \UncertaintySpeaker{k}(\msg \given \state) \StatePrior(\state)$
    \hfill (for $k > 1$)
  \end{examples}
\end{examples}

From the perspetive of this model, our decision to set $\lambda=1$ and
focus on $\UncertaintyListener[1]$ might appear arbitrary.  In
...
those values. These results suggest that further iteration beyond
$\UncertaintyListener[1]$ is not necessary \citep{Vogel-etal:2014},
and that $\lambda=1$ is too aggressive. (The cost of the null message
has a relatively small impact on the outcomes.) In addition, the
Unconstrained refinements model appears more viable for these minimal
settings; it consistently beats out the Neo-Gricean refinements,
though only by a narrow margin.
...
\begin{table}[H]
  ...
  \begin{tabular}[c]{ *{6}{r} }
    ...
    & MSE & .0069 & 1 & .1 & 1\\
    \addlinespace[1ex]
    \multirow{3}{*}{Unconstrained uncertainty} & Pearson & \graycell{.98} & 1 & .1 & 1\\
    & Spearman & \graycell{.81} & 1 & .2 & 1\\
    & MSE & \graycell{.0018} & 1 & .1 & 1\\
    \addlinespace[1ex]
    \multirow{3}{*}{Neo-Gricean uncertainty} & Pearson & .97 & 1 & .1 & 1\\
    & Spearman & .80 & 1 & .1 & 1\\
    & MSE & .0022 & 1 & .1 & 1\\
    \bottomrule
  \end{tabular}
...
catetories.  Our rationale for using this scale is that it allows
enough space for participants to both register a truth-value
assessment and convey information about the quality of the
report. This appendix reports briefly on this experiment.  The two
experiments yielded results that were identical in all important
respects.
...
The displays were identical to those in \figref{fig:exp-binary},
...
received between 19 and 44 responses (mean 30); this variation
...
the model. Neo-Gricean uncertainty again seems to be the best model,
though the results reported in
\appendixref{app:paramexplore} suggest that this victory is less
decisive than it would seem.
