...
speaker says \word{church or synagogue}, the listener receives a weak
...
represented by the set of sets in the gray box (`some, not all shots'). Our
...
$\sem{\word{some shot}} = \set{\set{a,b,c}}$ \Rogersmarginnote{Chris---I
  don't understand this `every player' thing---RPL}\magentahighlighter{(`every player')}, lexica
...
be identicallyy zero for simplicity.\footnote{The model is
  mathematically invariant to across-the-board additive
  transformations of message costs, so assuming all non-null messages
  to have zero cost loses no generality.} Our cost functions play a
role only in disfavoring the `null message' $\nullmsg$, which is
stipulated to be true in all worlds in all lexica.
...
lexicon in the set $\LexSet$.\footnote{$P(a \given b) \propto F(a)$ is
  ...
  the class of models defined in Equations~\ref{modobjects}.}
%
\begin{examples}
\item\label{agents}
  \begin{examples}
...
alternative cost function $C'$ ranging over $[0,\infty)$ such that
$C'(\msg)=e^{C(\msg)}$, and then replace Equation~\ref{s1} with
$\speakerOne(\msg | \state, \Lex) \propto C'(m)
\listenerZero(\state|\msg,\Lex)$.

\marginnote{This paragraph is new, responding to R1's queries about the null message.} 
\Rogersmarginnote{I moved this paragraph down from where Chris had it,
  and edited it.}
Some comment is in order regarding the role of the null message in the
model.  % This message can be thought of as gathering together all of
% the messages that are unusable in the context because they don't make
% any relevant distinctions, so the speaker remains silent if forced
% into this situation. 
...
$\msg$ such that $\state \in \Lex(\msg)$.  Without this, the speaker
distribution $\speakerOne(\msg | \state, \Lex)$ would not be defined.
There are a few alternative methods for addressing this technical
issue. \citet{Bergen:Goodman:Levy:2012} admit only lexica in which the
speaker has at least one true message for every state;
...
the iterated best-response model. The null-message approach has qualitatively similar behavior to these other approaches, and we favor it here because it is technically simpler to implement.\footnote{A closely related
  alternative, technically more complex but perhaps more
  pretheoretically transparent, would be to posit a collection of
  ...
  used to describe her observation state.  But the behavior of this
...
however, and in \appendixref{app:paramexplore} we thus explore their
consequences for modeling the pattern of data we observed.  First, one
...
behavior. The reader can consult \appendixref{app:paramexplore} for
fuller details.



% Ideally, we would explore the full
% space of reasonable models. Since the nature of this space is unknown,
% this is impossible. 

 % and their interaction with the cost assigned to
% the null message.
...
prior over states, but similar qualitative patterns would obtain using
a different prior specification.  Likewise, allowing lexical
...
the code provided with this paper. \footnote{There is one unnatural
  aspect of the simulation in \tabref{tab:sauerland}: it does not
  allow the speaker to be uncertain about the state space, whereas
  ...
  predicted interpretations of \word{Player~A hit some shot or
    cheered} and \word{Player~A hit some shot}. For a version of the
...
\begin{table}[tp]
  ...
  \caption{Nested scalar terms. For simplicity, we assume that there are just two
    ...
    in \world{s$_{1}$} Player~A made the first shot, missed the
    second, and didn't cheer; in \world{s$_{1}$s$_{2}$} Player~A
    made every shot but didn't cheer; in \world{c} is a world in which Player~A made no
    shots; in \world{--} Player~A made no shots and didn't cheer; and so forth.  The message of greatest
    ...
    that worlds inferred to have highest probability are those in which the
    player did not make all shots and did not both make shots and
    cheered (worlds \world{s$_1$} and \world{s$_{2}$}). This inference derives solely from competitions between
...
lexical uncertainty interacts with message competition (as in the example above)
...
\end{examples}
%
The results of working in this more constrained, \tech{Neo-Gricean}
...
\secref{sec:implicature}, it has long been assumed that scalar items
...
such inferences is critical in judging the suitability of this model as a description of human reasoning.
 % We now begin to pursue this
% question directly. 
The present section reviews the prior literature in this area.
% , which provides clear and fairly comprehensive guidance.

The pioneering paper in this area is
\citealt{Geurts:Pouscoulous:2009}. Their experiments 3 and 4 asked
participants to provide truth-value judgments for sentences paired
with abstract visual scenes consisting of shapes connected by
lines. The target sentences included weak scalar terms in upward,
downward, and non-monotone contexts, such as \emph{Exactly two of the
  squares are connected with some of the circles}, comparable in
relevant respects to the examples reviewed in \secref{sec:implicature}
above.  \citeauthor{Geurts:Pouscoulous:2009} found only negligible
rates of inferences consistent with local enrichment. These findings
stimulated a number of responses commenting on the prevalence of local
enrichment and its theoretical import
\citep{Ippolito:2010,Sauerland:2010}. The two responses that are most
relevant for our purposes are those of \citet{Clifton:Dube:2010} and
\citet{Chemla:Spector:2011}.
...
were criticized by \seccitet{geurts-vantiel:2013:scalar}{5.1} and
...
them.\footnote{For studies aimed at understanding the prevalence of
  implicatures, see \citealt{Paris:1973,Hendriks-etal:2009,degen:inpress-SP}.}
...
(`algorithms', in Marr's terminology) by which individuals perform
it. In particular, the averaged binary responses that we take as our
modeling target could come about via a number of routes. For example,
while individuals could be computing or approximating the full
computations that we describe here and then stochastically making
binary choices based on their estimates of the underlying probability
distribution. But they could also be pursuing any number of heuristic,
approximate strategies that --- when aggregated across individuals and
trials --- could yield a stable probability estimate. We remain agnostic about this issue here, but note that a growing literature explores these different hypotheses linking computational-level models to psychological processes (e.g., \citealt{bonawitz2014,griffiths2014,sanborn2010,vul2014}).
...
\item 
  ...
  \item \tech{Neo-Gricean refinement}: as in `Unconstrained
    ...
    \set{\sem{\word{some}}, \sem{\word{some and not all}}}$ as in
    Equation~\ref{neo} of Section~\ref{sec:illustrations}.
  \end{examples}
\end{examples}
...
once. However, we regard it as the simples viable linking hypothesis
...
The precise causes are hard to pinpoint, but the fact that our target
implicatures are not always enrichments of the literal content is
surely part of the problem.  \magentahighlighter{Second, while
  `Unconstrained uncertainty' is better than literal semantics by the
  Spearman and MSE metrics, it falls behind on the Pearson metric. We
  saw in connection with \tabref{tab:subjects} that the unintuitive
  ...
  poor performance here as well.}\Rogersmarginnote{Chris---this is no
  longer true for the binary-choice dataset, right?  Shall we just
  take this whole bit out?---Roger} Third, neo-Gricean uncertainty
...
illustrative example from \secref{sec:illustrations}, where we saw
...
validation. \Appendixref{app:paramexplore} offers additional relevant findings.
...
%
\Rogersmarginnote{\tiny Chris---I don't get
this.  Is the point that having \target{exactly one}{exactly one} as
an alternative blocks the local enrichment of \target{exactly
  one}{some} to be equivalent to what \target{exactly one}{some but
  not all} would mean?  If so, isn't this an artifact of the fact that
since our world space is defined in \texttt{N/S/A} terms, there is no
state-space distinction between \word{exactly one} and \word{some but
  not all} in the object position, even though a full treatment would
introduce such a distinction?}
%
\magentahighlighter{The only
