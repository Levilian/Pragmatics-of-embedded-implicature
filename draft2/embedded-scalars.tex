...
for all lexical items in $\Grammar$. For convenience we also assume
that $W$ is countable (so that probability mass can be put on each
world).
%
\begin{examples}
\item\label{modobjects}
  ...
  \item $\StatePrior : \wp(\Worlds) \mapsto [0,1]$ is a prior
    ...
    convenience we use $\StatePrior(\state)$ to mean
    $\StatePrior(\{\state\})$.)
  ...
  \item $\LexPrior : \wp(\LexSet) \mapsto [0,1]$ is a prior probability
    distribution over sets of lexica.  (For notational convenience we use $\LexPrior(\Lex)$ to mean
    $\LexPrior(\{\Lex\})$.)
  \end{examples}
\end{examples}
