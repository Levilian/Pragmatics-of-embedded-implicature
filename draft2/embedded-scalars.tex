...
of non-monotone quantifiers, as in \word{Exactly one player hit some
  of his shots}. In such cases, exhaustification of the embedded
...
\subeg{agents}{L} infers that the worlds with highest probability are
those in which the player did not make all shots and did not both make
shots and cheer, that is, worlds \world{s$_1$} and \world{s$_{2}$}. We
obtained the results of \tabref{tab:sauerland} using a uniform prior
...
\begin{table}[tp]
  ...
  \caption{Nested scalar terms. For simplicity, we assume that there are just two
    ...
    made every shot but didn't cheer; in \world{c}, Player~A made no
    ...
    \world{s$_1$} and \world{s$_{2}$} to have highest probability
...
\world{NN} (`neither player scored'). The lexical items are
\word{Player A}, \word{Player B}, \word{some}, \word{every},
...
allowing \word{only one} into object position has a major impact:
