...
\citeauthor{Bergen:Levy:Goodman:2014} in three respects. We give a more
complete treatment of semantic composition, we allow uncertainty in
the denotations of lexical items of a wider range of semantic types,
and we entertain the possibility of restrictions on the set of
possible refinements.  While many other elaborations of models in this 
space are possible \citep{Goodman:Lassiter:2013, Smith:Goodman:Frank:2013,
  Kao-etal:2014,Potts:Levy:2015}, our focus here on semantic composition limits 
the model variants that are relevant to investigate. 

There are two elaborations of the above
model that are highly salient given the prior literature, however, and thus
we explore their consequences for modeling the pattern of data we observed. 
First, one could allow further iteration
...
\citeauthor{Sauerland01} described. This suffices to capture the desired
...
rate of implicatures, and they sought to address this issue with a design in
which participants indicated the best match for the prompt sentence from a set of scenes.
In this setting, participants tended to
chose the scene consistent with local enrichment. This design is a
...
agent. The particulars of the experimental method were criticized by
\seccitet{geurts-vantiel:2013:scalar}{5.1} and \citet{vanTiel:2014}, however, on
...
complexity of the sentences involved, this choice seems likely to cause 
cognitive demands for participants in ways that could affect the stability and
...
Our assumption in this comparison is that our models provide a description of aggregate human behavior across individuals. In this sense, they are posed at \posscitet{marr1982} `computational theory' level. They instantiate claims abou the task that our participants are attempting to perform and the assumptions that they use in performing it, but they are agnostic about the particular processes (`algorithms', in Marr's terminology) by which individuals perform it. In particular, the averaged binary responses that we take as our modeling target could come about via a number of routes. For example, while individuals could be computing or approximating the full computations that we describe here and then stochastically making binary choices based on their estimates of the underlying probability distribution. But they could also be pursuing any number of heuristic, approximate strategies that --- when aggregated across individuals and trials --- could yield a stable probability estimate. We are agnostic about this issue here, but note that a growing literature explores these different hypotheses linking computational-level models to psychological processes (e.g., \citealt{bonawitz2014,griffiths2014,sanborn2010,vul2014}).
...
falls behind on the Pearson metric. We saw in connection with
